...
def print_all_pot():
    ...
    for i,pot in enumerate(cur_pots):
        pot_str += ("\n\n编号:" + str(i+1) + " 地点:" + pot.where + " 时间:" + pot.when + " 口味:" + pot.what + " 面数:" + str(sum(pot.noodle)) + " 饭数:" + str(sum(pot.rice)) + '\n乘客:')
...
async def jumpcar_got_target(bot: Bot, event: Event, state: T_State):
    ...
    if target >= len(cur_pots):
        target = len(cur_pots)
        newpot = one_pot(cur_pots[which].who[idx], cur_pots[which].where, cur_pots[which].when, cur_pots[which].what)
